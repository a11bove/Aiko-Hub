...
local function GetIcon(iconName)
    ...
    if LucideIcons and type(LucideIcons) == "table" and LucideIcons[lowerName] then
...
function AIKO:Window(GuiConfig)
    ...
    Close.Activated:Connect(function()
        ...
        Dialog.BackgroundColor3 = Color3.fromRGB(40, 20, 60)
        ...
        DialogGradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.0, Color3.fromRGB(60, 30, 90)),   -- Dark purple top
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(40, 20, 70)),   -- Darker middle
            ColorSequenceKeypoint.new(1.0, Color3.fromRGB(30, 15, 60))    -- Even darker bottom
        })
        DialogGradient.Rotation = 90  -- Vertical gradient
    ...
    -- Center title vertically if no content
    if ParagraphConfig.Content == "" or ParagraphConfig.Content == nil then
        ParagraphTitle.Position = UDim2.new(0, iconOffset, 0.5, -6)
    ...
    return ParagraphFunc
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                if ToggleConfig.Content == "" or ToggleConfig.Content == nil then
                    ToggleTitle.Position = UDim2.new(0, 10, 0.5, -6)
            ...
            function Items:AddSlider(SliderConfig)
                ...
                if SliderConfig.Content == "" or SliderConfig.Content == nil then
                    SliderTitle.Position = UDim2.new(0, 10, 0.5, -6)
    ...
    function DropdownFunc:AddOption(option)
        ...
        OptionButton.Activated:Connect(function()
            if DropdownConfig.Multi then
                ...
                if not found then
                    table.insert(DropdownFunc.Value, optionStr)
                end
            else
                DropdownFunc.Value = optionStr
            end
    ...
    function DropdownFunc:Set(Value)
        ...
        for _, Drop in ScrollSelect:GetChildren() do
            if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") then
                local optionText = Drop.OptionText.Text
                local selected = false

                if DropdownConfig.Multi then
                    for _, v in pairs(DropdownFunc.Value) do
                        if tostring(v) == optionText then
                            selected = true
                            break
                        end
                    end
                else
                    selected = tostring(DropdownFunc.Value) == optionText
                end
