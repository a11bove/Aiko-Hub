...
local TeleportModule = loadstring(game:HttpGet("https://raw.githubusercontent.com/a11bove/kdoaz/refs/heads/main/xzc/fishit/tpmdl.lua"))()

local function floatPlat(enabled)
    TeleportModule.floatPlat(enabled, LocalPlayer, EventTeleportSettings)
end

local function FindEventInWorkspace(eventName)
    return TeleportModule.FindEventInWorkspace(eventName)
end
...
loc:AddDropdown({
    ...
    Callback = function(selected)
        ...
        local targetPosition = TeleportModule.Locations[locationName]
